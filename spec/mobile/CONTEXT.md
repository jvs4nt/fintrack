...
- **Tab bar:** [`FinTrackTabBar`](../mobile/components/FinTrackTabBar.tsx) (wrapper + `BottomTabBar`) em [`app/(app)/_layout.tsx`](../mobile/app/(app)/_layout.tsx) — ícones Ionicons, rótulos Sora, `expo-haptics` no toque. Conteúdo em faixa fixa de **56px** centralizada; `paddingBottom: insets.bottom` no wrapper estende `Theme.bgSecondary` (#111118) até a base da tela (área do gesto/nav bar do sistema). iOS: blur no wrapper; Android: fundo sólido. `safeAreaInsets: { bottom: 0 }` nas tabs para não duplicar inset.
